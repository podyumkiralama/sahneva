...
const nextConfig = {
  ...
  // ⚠️ Next 13+’ta SWC minify zaten varsayılan; swcMinify anahtarı kaldırıldı (uyarı veriyordu)
  ...
  // Modern JS çıktısı ve paket optimizasyonu
  experimental: {
    // ❌ legacyBrowsers kaldırıldı (Next 15'te geçersizdi)
    // ❌ esmExternals çoğu projede gereksiz; kaldırıyoruz.
    // ✅ optimizePackageImports: sadece kullanılan modüller paketlerden alınır → daha küçük JS
    optimizePackageImports: [
      "lucide-react",
      // aşağıdakileri kullanıyorsanız burada bırakın, kullanmıyorsanız silebilirsiniz:
      "framer-motion",
      "@headlessui/react",
    ],
  },
  ...
  compiler: {
    // Prod'da console'ları temizle (error/warn hariç)
    removeConsole:
      process.env.NODE_ENV === "production" ? { exclude: ["error", "warn"] } : false,
    // ❌ transformMixedEsModules kaldırıldı (Next 15'te geçersizdi)
  },
    ...
    return [
      // 1) HTML & dinamik rotalar — güvenlik + 10 dk cache
      {
        ...
        headers: [
          ...
          // CSP — JSON-LD ve Next'in küçük inline scriptleri için 'unsafe-inline' gerekli
          {
